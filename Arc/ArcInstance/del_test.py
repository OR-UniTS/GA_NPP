...
class DelaunayInstanceTest(ArcInstance):
    def __init__(self, n_locations, n_tolls, dim_grid, toll_proportion,  n_commodities, costs=(5, 35), nr_users=(1, 5),
                 seed=False):
        # costs = (2, 20)
        if seed:
            np.random.seed(0)
        ...
        graph = nx.Graph()
        paths = []
        for sim in self.tri.simplices:
            paths.append(np.append(sim, sim[0]))
        for path in paths:
            nx.add_path(graph, path)

        self.pos = {node: points[i] for i, node in enumerate(graph.nodes)}

        # plt.triplot(points[:, 0], points[:, 1], tri.simplices)
        # plt.plot(points[:, 0], points[:, 1], 'o')
        # plt.show()
        #
        nx.draw(graph, pos={i: self.tri.points[i] for i in range(len(self.pos))}, with_labels=graph.nodes)
        plt.show()
        #
        ...
        for edge in graph.edges:
            graph.edges[edge]['weight'] = np.linalg.norm(self.pos[edge[0]]-self.pos[edge[1]])
    ...
    def show(self):
        nx.draw(self.npp, pos={i: self.tri.points[i] for i in range(len(self.pos))},
